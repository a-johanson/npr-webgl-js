...
export function stitchSegmentsToPolylines(segments) {
    function keyForPoint(p) {
        ...
        const q = (v) => Math.round(v * eps_inv);
        return `${q(p[0])};${q(p[1])}`;
    }
    // key -> array of connected points (store endpoints)
    // each endpoint maps to list of [otherEndpoint, segmentIndex]
    const endpointMap = new Map();

    segments.forEach(([p0, p1], i) => {
        const k0 = keyForPoint(p0), k1 = keyForPoint(p1);
    ...
    for (let i = 0; i < segments.length; i++) {
        if (used[i]) continue;
        // start new polyline from segments[i]
        let [a, b] = segments[i];
        used[i] = true;
        const poly = [[a[0], a[1]], [b[0], b[1]]];
        ...
        while (true) {
            ...
            for (const nb of neighbors) {
                ...
                if (!used[segIdx]) { found = { nb, segIdx }; break; }
            }
            if (!found) break;
            // append neighbor.other to poly
...
export function normalizedCurvaturesFromAngle(points, window = 10, minAngleDeg = 15.0, maxAngleDeg = 90.0) {
    ...
    for (let i = 1; i < n - 1; i++) {
        ...
        const v1 = [points[i][0] - points[start][0], points[i][1] - points[start][1]];
        const v2 = [points[end][0] - points[i][0], points[end][1] - points[i][1]];
...
export function laplacianSmoothing(points, curvatures, lambdaMax = 0.5, passes = 1, window = 1) {
    const n = points.length;
    ...
    for (let pass = 0; pass < passes; pass++) {
        const tmp = src; src = dest; dest = tmp;
        ...
        for (let i = 1; i < n - 1; i++) {
            ...
            const lambda = lambdaMax * (1.0 - curvatures[i]); // higher curvature -> less smoothing
            dest[i] = [
...
// points: Array<[x, y]>
// maxArea: threshold; points with effective area below this are removed
export function visvalingamWhyatt(points, maxArea) {
    function triangleArea(p1, p2, p3) { // Triangle area via cross product magnitude
        const ax = p2[0] - p1[0];
        const ay = p2[1] - p1[1];
        const bx = p3[0] - p1[0];
        const by = p3[1] - p1[1];
    ...
    // pointMetadata: index -> { area, version, prev, next }
    const pointMetadata = new Array(n);
    const heap = new MinHeap((a, b) => a.area - b.area); // entries: { area, idx, version }
    ...
    while (heap.size() && activeCount > 2) {
        const { area, idx, version }  = heap.pop(); // smallest area
        ...
        // Update prev point to bypass idx
        if (prevIdx !== null) {
            ...
            pointMetadata[prevIdx] = { area: newArea, version: newVersion, prev: prevPrev, next: nextIdx };
        }

        // Update next point to bypass idx
        if (nextIdx !== null) {
            ...
            pointMetadata[nextIdx] = { area: newArea, version: newVersion, prev: prevIdx, next: nextNext };
        }
    }
